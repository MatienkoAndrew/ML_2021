...
from filenames import Ui_Dialog as Ui_Dialog1
...
"}")
        ...
        self.checkBox_9.setGeometry(QtCore.QRect(10, 270, 221, 51))
...
"}")
        ...
        self.checkBox_10.setGeometry(QtCore.QRect(10, 320, 221, 41))
...
"}")
        ...
        self.checkBox_11.setGeometry(QtCore.QRect(10, 360, 241, 51))
...
"}")
        ...
        self.checkBox_12.setGeometry(QtCore.QRect(260, 230, 221, 41))
...
"}")
        ...
        self.checkBox_13.setGeometry(QtCore.QRect(260, 270, 241, 41))
...
"}")
        ...
        self.checkBox_14.setGeometry(QtCore.QRect(260, 320, 251, 41))
...
"}")
        ...
        self.checkBox_15.setGeometry(QtCore.QRect(260, 370, 161, 41))
...
"}")
        ...
        self.browse_2.setGeometry(QtCore.QRect(160, 440, 201, 60))
        font = QtGui.QFont()
        font.setPointSize(-1)
        font.setUnderline(False)
        font.setStrikeOut(False)
...
"}")
        ...
        self.progressBar.setGeometry(QtCore.QRect(10, 520, 491, 29))
...
"}")
        ...
        self.browse_3.setGeometry(QtCore.QRect(10, 560, 491, 60))
        font = QtGui.QFont()
        font.setPointSize(-1)
        font.setUnderline(False)
        font.setStrikeOut(False)
...
"}")
    ...
    def create_new(self):
        ...
        MainWindow.setMaximumSize(525, 100)
    ...
    def change_name(self, names):
        new_name = []
        for na in names:
            new_name.append(na.split('/')[-1])
        return new_name

    def browsefiles(self):
        self.name = \
            QFileDialog.getOpenFileNames(MainWindow, 'Open file', 'G:\ML_2021\\Nastya_Diplom',
                                         'Tables (*.xlsx , *.csv)')[0]
        self.pressed = True
        if self.pressed and self.name != []:
            ...
            self.animation.setEndValue(550)
    ...
    def create_file(self):
        ...
        self.animation1.setEndValue(580)
        self.animation1.start()

        ## Окно с названием файлов
        widget1 = QDialog()
        ui = Ui_Dialog1()
        ui.setupUi(widget1)
        widget1.setWindowTitle('Результаты обработки')
        widget1.setWindowIcon(QIcon('icon.jpg'))
        ...
        for filename in self.name:
            ...
            df.to_excel(f'Выгрузка\{filename1}_{current_datetime}.xlsx', index=False)
            self.progressBar.setValue(100)


        if len(error_massive) > 0:
            ui.label_2.setVisible(True)
            error_massive1 = self.change_name(error_massive)
            error_massive = str(error_massive1)
            error_massive = error_massive.replace("[", "")
            error_massive = error_massive.replace("]", "")
            error_massive = error_massive.replace("'", "")
            error_massive = error_massive.replace(",", "\n")
            _translate = QtCore.QCoreApplication.translate
            ui.label_2.setText(_translate("MainWindow", f"Некорректные файлы:\n{error_massive}"))
            # QMessageBox.critical(MainWindow, "Ошибка ",
            #                  f"Некорректные данные в файле:\n{error_massive}", QMessageBox.Ok)
        ...
        self.animation2.setStartValue(580)
        self.animation2.setEndValue(675)
        self.animation2.start()
        self.browse_3.clicked.connect(self.open_directory)


        self.filename = self.change_name(self.name)
        if len(error_massive) > 0:
            a = list(set(self.filename) - set(error_massive1))
            if len(a) > 0:
        ...
        _translate = QtCore.QCoreApplication.translate
        ui.label.setText(_translate("MainWindow", f"Обработанные файлы:\n{b}"))
        ui.pushButton.clicked.connect(lambda: widget1.close())
        widget1.exec_()
